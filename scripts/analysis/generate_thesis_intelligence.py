...
ARTIFACTS = {
    ...
    "t11_cqr": Path(
        "results/predictions/point_net_transf_gat_11th_trial_cqr_frozen/val_predictions.npz"
    ),
    "temperature": Path("results/temperature_scaling_t8.json"),
    "conditional_conformal": Path("results/conformal_conditional_coverage_t8.json"),
    "t7_point_metrics": Path(
        "results/trials/point_net_transf_gat_7th_trial_80_10_10_split/"
        "test_evaluation_complete.json"
    ),
    "t8_point_metrics": Path(
        "results/trials/point_net_transf_gat_8th_trial_lower_dropout/"
        "test_evaluation_complete.json"
    ),
    "deep_ensemble_metrics": Path(
        "results/trials/deep_ensemble_results/ensemble_metrics.json"
    ),
    "t8_verified_metrics": Path(
        "results/trials/uq_verification_run/mc_dropout_verified_metrics.json"
    ),
    "t8_verified_predictions": Path(
        "results/predictions/uq_verification_run/mc_dropout_verified.npz"
    ),
    "t10_metrics": Path(
        "results/trials/point_net_transf_gat_10th_trial_cqr/"
        "cqr_results/cqr_metrics.json"
    ),
    "t11_metrics": Path(
        "results/trials/point_net_transf_gat_11th_trial_cqr_frozen/"
        "cqr_results/cqr_metrics.json"
    ),
}

LOCAL_TEST_LOADER = Path(
...
DISCREPANCIES = [
    {
        ...
        "resolution": "Corrected target-zero claims in the LaTeX thesis source.",
    },
    {
        "severity": "high",
        ...
        rows.append(
            {
                ...
                "trust_boundary": "tracked repository artifact",
            }
        )
    ...
    rows.append(
        {
            ...
            "sha256": None,
            "trust_boundary": "ignored local-only pickle-capable artifact; never export",
        }
    )
    return rows
...
def main() -> int:
    args = parse_args()
    missing = [path.as_posix() for path in ARTIFACTS.values() if not (ROOT / path).is_file()]
    if missing:
        raise FileNotFoundError(f"Required tracked artifacts are missing: {', '.join(missing)}")
