...
def find_best_mc_npz(uq_dir):
    ...
    print(
        f"  Selected NPZ: {os.path.basename(best)} ({os.path.getsize(best) / 1e6:.1f} MB)"
    )
    return best
...
def compute_conformal_metrics(
    targets, predictions, uncertainties, split_ratio=0.5, seed=42
):
    """
...
def plot_pred_vs_target(targets, predictions, model_num, save_path, sample_size=50000):
    ...
    ax.text(
        ...
        bbox=dict(
            boxstyle="round", facecolor="white", alpha=0.9, edgecolor=COLORS["light"]
        ),
    )
...
def plot_uncertainty_vs_error_scatter(
    targets, predictions, uncertainties, model_num, save_path, sample_size=50000
):
...
def plot_binned_error_vs_uncertainty(
    targets, predictions, uncertainties, model_num, save_path, n_bins=20
):
...
def plot_coverage_comparison(conformal_results, model_num, save_path):
    ...
    bars1 = ax.bar(
        x - width / 2, cov_90, width, label="90% nominal", color="steelblue", alpha=0.8
    )
    bars2 = ax.bar(
        x + width / 2, cov_95, width, label="95% nominal", color="coral", alpha=0.8
    )
...
def plot_uq_dashboard(
    ...
    for bar, cov in zip(bars, coverages):
        if not np.isnan(cov):
            ax.annotate(
                f"{cov:.1f}%",
                xy=(bar.get_x() + bar.get_width() / 2, cov),
                xytext=(0, 3),
                textcoords="offset points",
                ha="center",
                va="bottom",
...
def generate_standard_uq_plots(
    model_dir, model_num, sample_size=50000, split_ratio=0.5, seed=42
):
    """
    ...
    conformal = compute_conformal_metrics(
        targets, predictions, uncertainties, split_ratio=split_ratio, seed=seed
    )

    print(
        f"  Absolute method - 90%: q={conformal['absolute_q_90']:.4f}, PICP={conformal['absolute_picp_90']:.2f}%"
    )
    print(
        f"  Absolute method - 95%: q={conformal['absolute_q_95']:.4f}, PICP={conformal['absolute_picp_95']:.2f}%"
    )
    print(
        f"  Sigma-norm method - 90%: k={conformal['sigma_k_90']:.4f}, PICP={conformal['sigma_picp_90']:.2f}%"
    )
    print(
        f"  Sigma-norm method - 95%: k={conformal['sigma_k_95']:.4f}, PICP={conformal['sigma_picp_95']:.2f}%"
    )

    # Save conformal metrics
    conformal_save = {
        k: v for k, v in conformal.items() if not isinstance(v, np.ndarray)
    }
    ...
    plot_pred_vs_target(
        ...
        predictions,
        model_num,
        os.path.join(plot_dir, "pred_vs_target.png"),
        sample_size,
    )

    plot_uncertainty_hist(
        uncertainties, model_num, os.path.join(plot_dir, "uncertainty_hist.png")
    )

    plot_error_hist(
        targets, predictions, model_num, os.path.join(plot_dir, "error_hist.png")
    )
    ...
    plot_uncertainty_vs_error_scatter(
        ...
        uncertainties,
        model_num,
        os.path.join(plot_dir, "uncertainty_vs_error_scatter.png"),
        sample_size,
    )

    plot_binned_error_vs_uncertainty(
        targets,
        predictions,
        uncertainties,
        model_num,
    ...
    plot_coverage_curve_k_sigma(
        conformal, model_num, os.path.join(plot_dir, "coverage_curve_k_sigma.png")
    )

    plot_coverage_comparison(
        conformal, model_num, os.path.join(plot_dir, "coverage_comparison.png")
    )
    ...
    plot_uq_dashboard(
        ...
        conformal,
        model_num,
        os.path.join(plot_dir, "uq_dashboard.png"),
        sample_size,
    )
...
Examples:
  ...
  python plot_uq_standard.py --model-dir data/TR-C_Benchmarks/point_net_transf_gat_8th_trial_lower_dropout --model-num 8
    ...
    parser.add_argument(
        "--model-num", type=int, required=True, help="Model number for plot titles"
    )
    parser.add_argument(
        "--sample-size", type=int, default=50000, help="Max points in scatter plots"
    )
    parser.add_argument(
        "--split", type=float, default=0.5, help="Calibration/test split ratio"
    )
