#!/usr/bin/env python3
"""
UQ Analysis - Using Pre-computed Data
====================================================
...
def load_precomputed_data():
    ...
    npz_path = os.path.join(
        MODEL_FOLDER, "uq_results/mc_dropout_full_100graphs_mc30.npz"
    )
...
def analysis_1_threshold_decision(results, output_dir):
    ...
    for p, thresh in zip(percentiles, thresholds):
        ...
        print(
            f"  {100 - p}% confidence → {pct_flagged:.1f}% flagged | MAE: {unflagged_mae:.1f} (kept) vs {flagged_mae:.1f} (flagged)"
        )
    ...
    for p in [90, 95]:
        if p in pct_kept:
            ...
            ax.annotate(
                f"{p}%: MAE={mae_kept[idx]:.1f}",
                xy=(p, mae_kept[idx]),
                xytext=(p - 15, mae_kept[idx] + 8),
                fontsize=9,
                fontweight="bold",
            )
...
def analysis_2_uncertainty_heatmap(results, output_dir):
    ...
    cmap_unc = LinearSegmentedColormap.from_list(
        "unc", ["#E8F4F8", "#B8D4E8", "#7EB5D6", "#4A90B8", "#2C5F7C"]
    )
    cmap_err = LinearSegmentedColormap.from_list(
        "err", ["#E8F8E8", "#B8E8B8", "#7ED67E", "#4AB84A", "#2C7C2C"]
    )
    ...
    scatter = ax.scatter(
        x,
        y,
        c=np.clip(unc_plot, 0, np.percentile(unc_plot, 99)),
        cmap=cmap_unc,
        s=1,
        alpha=0.6,
    )
    ...
    ax.set_title(
        "(a) Uncertainty Map - Paris Network",
        fontweight="bold",
        color=COLORS["text"],
        fontsize=12,
    )
    ...
    scatter = ax.scatter(
        x,
        y,
        c=np.clip(err_plot, 0, np.percentile(err_plot, 99)),
        cmap=cmap_err,
        s=1,
        alpha=0.6,
    )
...
def analysis_3_feature_error(results, output_dir):
    ...
    for i in range(n_features):
        ...
        print(
            f"  {FEATURE_NAMES[i]:<25}: ρ_unc = {corr_unc:+.4f}, ρ_err = {corr_err:+.4f}"
        )
    ...
    for i in range(n_features):
        ...
        ax.set_title(
            f"ρ = {feature_stats[i]['corr_uncertainty']:+.3f}",
            fontweight="bold",
            color=COLORS["text"],
        )
        ax.grid(True, alpha=0.3)
...
def analysis_4_calibration(results, output_dir):
    ...
    for i in range(n_bins):
        mask = (uncertainties >= bin_edges[i]) & (
            uncertainties < bin_edges[i + 1]
            if i < n_bins - 1
            else uncertainties <= bin_edges[i + 1]
        )
        if np.sum(mask) > 0:
    ...
    ece = sum(
        cd["n_samples"]
        / total
        * abs(cd["mean_uncertainty"] - cd["mean_error"] / np.mean(abs_errors))
    ...
    prev_q = 0
    for q in quartiles:
        mask = (uncertainties >= np.percentile(uncertainties, prev_q)) & (
            uncertainties <= np.percentile(uncertainties, q)
        )
    ...
    ax.set_title(
        "(b) Error by Confidence Quartile", fontweight="bold", color=COLORS["text"]
    )
    ...
    ax.fill_between(
        cum_pct[sample], cum_mae[sample], alpha=0.3, color=COLORS["sky_blue"]
    )
    for p in [50, 90]:
        i = int(len(sorted_err) * p / 100)
        ax.scatter(
            [p], [cum_mae[i]], s=100, color=COLORS["rose"], edgecolor="white", zorder=5
        )
        ax.annotate(
            f"{p}%: {cum_mae[i]:.1f}",
            xy=(p, cum_mae[i]),
            xytext=(p + 3, cum_mae[i] + 5),
            fontweight="bold",
        )
    ax.set_xlabel(
...
def main():
    print("=" * 80)
    print("UQ ANALYSIS (Using Pre-computed Data)")
    print("=" * 80)
