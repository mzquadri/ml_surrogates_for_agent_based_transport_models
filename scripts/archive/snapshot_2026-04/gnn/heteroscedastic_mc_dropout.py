...
def heteroscedastic_mc_dropout(
    ...
    iterator = tqdm(data_loader, desc='Heteroscedastic MC Dropout') if verbose else data_loader
...
def analyze_uncertainty_decomposition(results: Dict[str, np.ndarray]) -> Dict[str, float]:
    """
...
def compute_calibration_metrics(results: Dict[str, np.ndarray]) -> Dict[str, float]:
    ...
    Returns:
        dict: Calibration metrics:
            ...
            - k_95: Calibration factor for 95% coverage (compare to baseline 11.34)
...
def compare_with_baseline(
    het_results: Dict[str, np.ndarray],
    baseline_locked_path: str
