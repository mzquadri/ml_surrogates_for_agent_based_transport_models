...
MODEL_DIR = f"{BASE_PATH}/data/TR-C_Benchmarks/pointnet_transf_gat_bs32_5feat_seed42"
...
existing_results_path = f"{MODEL_DIR}/test_results.json"
if os.path.exists(existing_results_path):
    print(f"✓ Found existing test results: {existing_results_path}")
    with open(existing_results_path, 'r') as f:
        existing_results = json.load(f)
    print(f"\nExisting Test Metrics:")
    print(f"  R² Score: {existing_results.get('r2_score', 'N/A')}")
    print(f"  Pearson:  {existing_results.get('pearson_correlation', 'N/A')}")
    print(f"  MAE:      {existing_results.get('mae', 'N/A')}")
    print(f"\nProceeding with fresh evaluation...\n")
else:
...
# ============================================================================
print("="*80)
print(" LOADING MODEL - BATCH SIZE 32 (5 FEATURES, SEED 42)")
print("="*80)
...
# Load model (NOTE: LEGACY ARCHITECTURE - like Trial 2, no final GAT layer)
print("⚠️  WARNING: This model uses LEGACY architecture (pre-Trial 3)")
print("    - No final GAT layer")
print("    - Different layer indexing")
print("    - Uses read_out_node_predictions instead of gat_final")
...
# Load with strict=False to allow architecture differences
state_dict = torch.load(f"{MODEL_DIR}/model.pth", map_location=device)
incompatible_keys = model.load_state_dict(state_dict, strict=False)

if incompatible_keys.missing_keys:
    print(f"⚠️  Missing keys (expected in current architecture): {len(incompatible_keys.missing_keys)} keys")
if incompatible_keys.unexpected_keys:
    print(f"⚠️  Unexpected keys (from legacy model): {len(incompatible_keys.unexpected_keys)} keys")

model.eval()
print("\n✓ Model loaded successfully (LEGACY architecture with partial weights)!\n")
...
# Load dataset (NOTE: Different path - /dataloaders/ folder)
test_dataset = torch.load(f"{MODEL_DIR}/dataloaders/test_dl.pt", weights_only=False)
print(f"✓ Test dataset loaded: {len(test_dataset)} samples")

# Load loader params (NOTE: Path has typo - no slash between 'dataloaders' and 'test')
loader_params_path = f"{MODEL_DIR}/dataloaderstest_loader_params.json"
if not os.path.exists(loader_params_path):
    # Try alternative path
    loader_params_path = f"{MODEL_DIR}/dataloaders/test_loader_params.json"

with open(loader_params_path, 'r') as f:
...
loss_fn = GNN_Loss("mse", num_nodes, device, False)  # No weighted loss
...
print(f"{'='*40}")
print(f"{'Metric':<20} {'Trial 5':<15} {'BS32 Model':<15} {'Difference':<15}")
print(f"{'-'*65}")
...
print(f"{'Batch Size':<20} {'8 (eff:24)':<15} {'32':<15} {'+300%':<15}")
...
ax.set_title('Batch Size Experiment Comparison', fontsize=14, fontweight='bold')
ax.set_ylim(0, 0.85)
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3, axis='y')
...
plt.suptitle('Batch Size 32 Model - Test Set Evaluation', 
...
# ============================================================================
results = {
    "trial_name": "bs32_5feat_seed42",
    "hyperparameters": {
        "batch_size": 32,
        "gradient_accumulation": "unknown",
        "use_dropout": True,
        "dropout": 0.3,
...
print("="*80)
print("\nNEXT STEP: Compare BS32 results with Trial 5 to determine impact of batch size")
