...
MODEL_DIR = f"{BASE_PATH}/data/TR-C_Benchmarks/point_net_transf_gat_8th_trial_lower_dropout"

# ============================================================================
# LOAD MODEL
# ============================================================================
print("="*80)
print(" LOADING MODEL - 8TH TRIAL (LOWER DROPOUT 0.2)")
print("="*80)
...
print(f"{'-'*50}")
print(f"{'R² Score':<20} {0.5970:<15.4f} {r2:<15.4f}")
print(f"{'Pearson':<20} {0.7734:<15.4f} {pearson_r:<15.4f}")
print(f"{'Spearman':<20} {0.2912:<15.4f} {spearman_r:<15.4f}")
print(f"{'Loss (MSE)':<20} {49.27:<15.2f} {avg_test_loss:<15.2f}")
...
val_r2 = 0.5970
test_r2 = r2
r2_gap = val_r2 - test_r2
gap_percent = (r2_gap / val_r2) * 100

print(f"\n1. GENERALIZATION GAP:")
print(f"   Val R²:  {val_r2:.4f}")
print(f"   Test R²: {test_r2:.4f}")
print(f"   Gap:     {r2_gap:+.4f} ({gap_percent:+.1f}%)")

if abs(gap_percent) < 5:
    print(f"   Status:  ✅ Good generalization (gap < 5%)")
    overfit_status = "NO OVERFITTING"
elif gap_percent > 5:
    print(f"   Status:  ⚠️  Possible overfitting (val > test by {gap_percent:.1f}%)")
    overfit_status = "SLIGHT OVERFITTING"
else:
    print(f"   Status:  ✅ Test better than val (unusual but good!)")
    overfit_status = "NO OVERFITTING"

# 2. Variance analysis
target_std = np.std(all_targets)
pred_std = np.std(all_predictions)
variance_ratio = pred_std / target_std * 100

print(f"\n2. VARIANCE COVERAGE:")
print(f"   Target Std:     {target_std:.4f}")
print(f"   Prediction Std: {pred_std:.4f}")
print(f"   Coverage:       {variance_ratio:.1f}%")

if variance_ratio < 70:
    print(f"   Status:  ⚠️  Underfitting (predictions too conservative)")
    underfit_status = "STILL UNDERFITTING"
elif variance_ratio > 110:
    print(f"   Status:  ⚠️  Possible overfitting (predictions too varied)")
    underfit_status = "POSSIBLY OVERFITTING"
else:
    print(f"   Status:  ✅ Good variance coverage")
    underfit_status = "OPTIMAL FIT"

print(f"\n{'='*40}")
print(f"  FINAL DIAGNOSIS")
print(f"{'='*40}")
print(f"Overfitting:  {overfit_status}")
print(f"Underfitting: {underfit_status}")

if overfit_status == "NO OVERFITTING" and underfit_status in ["OPTIMAL FIT", "STILL UNDERFITTING"]:
    print(f"\n✅ VERDICT: Model is well-balanced for thesis use!")
else:
    print(f"\n⚠️  VERDICT: Consider further tuning")

# ============================================================================
# COMPARISON WITH ALL TRIALS
# ============================================================================
print(f"\n{'='*80}")
print(f"  COMPARISON: ALL TRIALS (TEST SET)")
print(f"{'='*80}")
print(f"{'Trial':<15} {'R²':<10} {'Pearson':<10} {'MAE':<10} {'Status':<15}")
print(f"{'-'*70}")
print(f"{'Trial 5':<15} {0.5553:<10.4f} {0.7468:<10.4f} {4.2421:<10.4f} {'Baseline':<15}")
print(f"{'Trial 6':<15} {0.5223:<10.4f} {0.7262:<10.4f} {4.3242:<10.4f} {'Lower LR (-)':<15}")
print(f"{'Trial 7':<15} {0.5471:<10.4f} {0.7409:<10.4f} {4.0601:<10.4f} {'Higher LR (-)':<15}")
print(f"{'Trial 8':<15} {r2:<10.4f} {pearson_r:<10.4f} {mae:<10.4f} {'Lower Dropout':<15}")

if r2 > 0.5553:
    print(f"\n🏆 TRIAL 8 IS THE NEW BEST MODEL!")
else:
    print(f"\n⚠️  Trial 5 still better - accept Trial 5 as final")
...
# 4. All Trials Comparison
ax = axes[1, 1]
trials = ['Trial 5\n(drop=0.3)', 'Trial 6\n(LR low)', 'Trial 7\n(LR high)', 'Trial 8\n(drop=0.2)']
r2_scores = [0.5553, 0.5223, 0.5471, r2]
colors = ['#ffd93d', '#ff6b6b', '#ff6b6b', '#51cf66' if r2 > 0.5553 else '#ffd93d']
...
ax.set_title('All Trials Comparison (Test Set)', fontsize=14, fontweight='bold')
ax.set_ylim(0, 0.85)
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3, axis='y')
...
plt.suptitle('8th Trial (Lower Dropout=0.2) - Test Set Evaluation', 
...
# ============================================================================
results = {
    "trial_name": "8th_trial_lower_dropout",
    "hyperparameters": {
        "batch_size": 8,
    ...
    "diagnosis": {
        ...
        "val_test_gap_percent": float(gap_percent),
        "variance_coverage_percent": float(variance_ratio)
    },
    "comparison_with_trial5": {
        "trial5_r2": 0.5553,
        "trial8_r2": float(r2),
        "improvement": float(r2 - 0.5553),
        "percentage_improvement": float((r2 - 0.5553) / 0.5553 * 100)
    }
}
