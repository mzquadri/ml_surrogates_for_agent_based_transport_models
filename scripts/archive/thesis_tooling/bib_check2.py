...
# More precise pattern: match \cite, \citep, \citet followed by { and alphanumeric keys
...
print('=== CHECK 1: ENTRY COUNTS BY TYPE ===')
...
print('  Expected: 48 -> ' + ('PASS' if total_target == 48 else 'FAIL'))
print()

print('=== CHECK 2: Extracted citation keys ===')
print('  Total unique keys: ' + str(len(cite_keys)))
print()

missing = cite_keys - set(bib_entries.keys())
print('=== CHECK 3: CITED BUT NOT IN BIB (missing entries) ===')
if missing:
...
print('=== CHECK 4: IN BIB BUT NOT CITED (orphaned entries) ===')
if orphaned:
...
for key in ['hasanzadeh2020bayesian', 'zhang2019bayesian']:
    ...
    if title_m:
        ...
        if '{B}ayesian' in title:
            print('  ' + key + ': PASS (title contains {B}ayesian)')
        else:
