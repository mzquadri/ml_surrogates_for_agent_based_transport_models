...
def main():
    try:
        ...
        while True: # Change this to "and batch_num < 10" for a faster run
            ...
            if not os.path.exists(batch_file):  # Safety check to avoid infinite loop
                break
        ...
        print(f"Sample data item: {datalist[0]}")
        print(datalist[0].x)
    ...
    parser.add_argument("--unique_model_description", type=str, default="",
    ...
    try:
        ...
        unique_model_description = args["unique_model_description"] if args["unique_model_description"] != "" else args["gnn_arch"]
        unique_model_description += f"_5_features" if not args["use_all_features"] else f"_{len(EdgeFeatures)}_features"
        
        # Create directory for the run
        unique_run_dir = os.path.join(base_dir, args['project_name'], unique_model_description)
        os.makedirs(unique_run_dir, exist_ok=True)

        model_save_path, path_to_save_dataloader = get_paths(base_dir=os.path.join(base_dir, args['project_name']), unique_model_description=unique_model_description, model_save_path='trained_model/model.pth')
