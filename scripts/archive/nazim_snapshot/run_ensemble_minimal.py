"""
Minimal ensemble experiment: 3 graphs, 3 runs, S=10.
Purpose: Get corrected Spearman rho values to confirm fix works.
Usage: conda run -n thesis-env python scripts/run_ensemble_minimal.py
"""
...
def mc_dropout_predict(model, data, S=10):
    model.train()
...
if __name__ == "__main__":
    ...
    for run in range(N_RUNS):
        ...
        print("  Run %d/%d (%.1fs)" % (run + 1, N_RUNS, time.time() - t_run))
        sys.stdout.flush()
    ...
    print("    Ensemble: R2=%.4f MAE=%.4f rho=%.4f" % (r2_b, np.mean(abs_err_b), rho_b))
    print()
