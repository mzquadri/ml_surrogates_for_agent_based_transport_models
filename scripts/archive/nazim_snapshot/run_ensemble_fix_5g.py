...
Reduced to 5 graphs to fit in CPU time budget.
5 graphs x 31635 nodes = 158,175 nodes.
Usage: conda run -n thesis-env python scripts/run_ensemble_fix_5g.py
"""
...
if __name__ == "__main__":
    ...
    print("  T8 model loaded. Starting MC Dropout runs...")
    ...
    for run in range(N_RUNS):
        ...
        print(
            "  Run %d/%d done (%.1fs) [total %.1fs]"
            % (run + 1, N_RUNS, elapsed_run, time.time() - t0)
        )
        sys.stdout.flush()
    ...
    print("    R2  = %.4f, MAE = %.4f" % (r2_a, np.mean(abs_err)))
    ...
    # Load models one at a time to save memory, run all graphs, then free
    all_preds_b = {m: [] for m in model_nums}
    all_targets_b = []

    # First pass: collect targets
    for gi in range(N_GRAPHS):
        data = test_dl[gi]
        all_targets_b.append(data.y.squeeze().cpu().numpy())

    targets_b = np.concatenate(all_targets_b)

    for m in model_nums:
        t_m = time.time()
        print("  Loading and running T%d..." % m)
        model = load_model_fixed(m)
        model.eval()
        preds_m = []
        with torch.no_grad():
            for gi in range(N_GRAPHS):
        ...
        all_preds_b[m] = np.concatenate(preds_m)
        del model
        gc.collect()
        print("    T%d done (%.1fs)" % (m, time.time() - t_m))
        sys.stdout.flush()
    ...
    rho_b = spearmanr(ens_unc, abs_err_b)[0]
    ...
    print(
        "    Ensemble: R2=%.4f, MAE=%.4f, rho=%.4f" % (r2_b, np.mean(abs_err_b), rho_b)
    )
    print()
