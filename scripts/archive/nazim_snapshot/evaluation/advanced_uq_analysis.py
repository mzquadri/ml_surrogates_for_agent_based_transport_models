"""
Advanced UQ Analysis: Key Thesis Visualizations
=====================================================
...
def plot_binned_error_vs_sigma(data, model_num, save_dir, n_bins=20):
    """
    Binned error vs sigma with quantile bands.
    ...
    ax.text(
        0.05,
        0.85,
        f"Top 10% σ → {top10_error_share:.1f}% of total error",
        transform=ax.transAxes,
        fontsize=10,
        va="top",
    ...
    ax.set_title(
        f"Model {model_num}: Binned Error vs Uncertainty\n"
        f"(Higher σ → Higher Error: Uncertainty is Informative)",
        fontsize=13,
    )
    ...
    print(
        f"    Spearman ρ = {rho:.4f}, Top 10% σ accounts for {top10_error_share:.1f}% of error"
    )
...
def plot_risk_coverage_curve(data, model_num, save_dir, n_points=100):
    """
    Error vs retained fraction after filtering by σ.
    ...
    ax.set_title(
        f"Model {model_num}: Risk-Coverage Curve\n"
        f"(Rejecting high-σ samples reduces error)",
        fontsize=13,
    )
    ...
    print(
        f"    50% retention → MAE {mae_at_50:.2f} ({improvement_50:.1f}% improvement)"
    )
    print(
        f"    90% retention → MAE {mae_at_90:.2f} ({improvement_90:.1f}% improvement)"
    )
...
def plot_interval_width_comparison(data, model_num, save_dir):
    """
    Compare interval widths - absolute vs sigma-normalized.
    ...
    ax.hist(
        ...
        alpha=0.7,
        color=COLORS["uncertainty"],
        label=f"σ-normalized (mean={sigma_mean:.1f})",
        density=True,
    )
    ax.axvline(
        abs_mean,
    ...
    ax.axvline(sigma_mean, color=COLORS["uncertainty"], linestyle="-", lw=2, alpha=0.7)
    ...
    ax.set_title(
        "σ-Normalized Intervals are ADAPTIVE\n(narrow for confident, wide for uncertain)",
        fontsize=13,
    )
    ax.legend(loc="upper left")
    ...
    # Annotations
    ax.annotate(
        f"Low σ: {sigma_width_low:.1f}\n({efficiency_low:.0f}% narrower)",
        xy=(1, sigma_width_low),
        xytext=(1.3, sigma_width_low + 5),
        fontsize=9,
        ha="left",
        arrowprops=dict(arrowstyle="->", color="gray"),
    )
    ...
    print(
        f"    Low-σ (Q1) width: {sigma_width_low:.2f} ({efficiency_low:.1f}% narrower than absolute)"
    )
...
def plot_calibration_diagnostic(data, model_num, save_dir):
    ...
    ax.set_title(
        f"Model {model_num}: Calibration Diagnostic\n"
        f"σ is UNDER-DISPERSED (k₉₅={k95:.1f} >> 1.96)",
        fontsize=13,
    )
    ...
    print(
        f"    Mean σ = {mean_sigma:.2f}, RMSE = {rmse:.2f}, k95 = {k95:.2f} (should be ~1.96)"
    )
