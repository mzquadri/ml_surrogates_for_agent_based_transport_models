...
def load_model(model_folder: str, model_num: int, device: torch.device):
    """Load trained model with GATConv weight remapping fix.

    Old PyG checkpoints store GATConv weights as 'lin.weight',
    but PyG 2.3.1 expects 'lin_src.weight' and 'lin_dst.weight'.
    We remap the keys before loading with strict=True.
    """
    ...
    # Remap old PyG GATConv keys: lin.weight -> lin_src.weight + lin_dst.weight
    remapped = {}
...
def run_experiment_a(device, num_ensemble_runs=5, mc_samples=30, max_graphs=10):
    ...
    results = {
        ...
        "mc_dropout_uncertainty": compute_metrics(
            ensemble_mean_pred, all_targets, avg_mc_uncertainty
        ),
        "ensemble_variance": compute_metrics(
            ensemble_mean_pred, all_targets, ensemble_variance
        ),
        "combined_uncertainty": compute_metrics(
            ensemble_mean_pred, all_targets, combined_uncertainty
        ),
    }
    ...
    print(
        f"  MC Dropout σ:       {results['mc_dropout_uncertainty']['spearman_corr']:.4f}"
    )
    print(f"  Ensemble Variance:  {results['ensemble_variance']['spearman_corr']:.4f}")
    print(
        f"  Combined:           {results['combined_uncertainty']['spearman_corr']:.4f}"
    )
    print()
    print("Uncertainty Statistics:")
    print(
        f"  MC Dropout σ:       mean={results['mc_dropout_uncertainty']['uncertainty_mean']:.4f}, std={results['mc_dropout_uncertainty']['uncertainty_std']:.4f}"
    )
    print(
        f"  Ensemble Variance:  mean={results['ensemble_variance']['uncertainty_mean']:.4f}, std={results['ensemble_variance']['uncertainty_std']:.4f}"
    )
    print(
        f"  Combined:           mean={results['combined_uncertainty']['uncertainty_mean']:.4f}, std={results['combined_uncertainty']['uncertainty_std']:.4f}"
    )
    print()
...
def plot_experiment_a(
    output_folder, targets, predictions, mc_unc, ens_var, combined_unc
):
    ...
    sample_idx = np.random.choice(
        len(targets), size=min(10000, len(targets)), replace=False
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(plot_folder, "exp_a_uncertainty_distributions.png"), dpi=150
    )
    plt.close()
...
def run_experiment_b(device, max_graphs=10, use_weighted=True):
    ...
    test_dl_path = os.path.join(
        ref_model_folder, "data_created_during_training/test_dl.pt"
    )
    test_set_dl = torch.load(test_dl_path, weights_only=False)
    n_graphs = min(len(test_set_dl), max_graphs)
    ...
    for model_num in model_nums:
        ...
        model_folder = os.path.join(
            REPO_ROOT, "data/TR-C_Benchmarks", MODEL_FOLDERS[model_num]
        )
    ...
    for model_num in model_nums:
        ...
        print(
            f"  Model {model_num}: R²={metrics['r2']:.4f}, MAE={metrics['mae']:.4f}, RMSE={metrics['rmse']:.4f}"
        )
    ...
    best_model = max(
        model_nums, key=lambda m: results["individual_models"][str(m)]["r2"]
    )
...
def plot_experiment_b(
    ...
    r2_values = [results["individual_models"][str(m)]["r2"] for m in model_nums] + [
        results["ensemble"]["r2"]
    ]
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(plot_folder, "exp_b_uncertainty_distribution.png"), dpi=150
    )
    plt.close()
    ...
    ax = axes[5]
    ax.scatter(
        targets[sample_idx], ensemble_pred[sample_idx], alpha=0.3, s=1, c="green"
    )
    max_val = max(np.percentile(targets, 99), np.percentile(ensemble_pred, 99))
    ax.plot([0, max_val], [0, max_val], "r--")
    ax.set_xlabel("Target")
...
def main():
    ...
    parser.add_argument(
        "--mc-samples", type=int, default=30, help="MC samples per run (default: 30)"
    )
    parser.add_argument(
        "--cpu", action="store_true", help="Force CPU even if GPU available"
    )
    ...
    if args.experiment in ["B", "both"]:
        results["experiment_b"] = run_experiment_b(
            device=device, max_graphs=args.max_graphs, use_weighted=True
        )
        print()
