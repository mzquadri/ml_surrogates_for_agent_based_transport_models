...
def run_inference(model, test_data, device, n_graphs=None, mc_samples=50):
    ...
    print(
        f"Running MC Dropout inference on {n_graphs} graphs with {mc_samples} samples..."
    )
...
def analysis_1_threshold_decision(results, output_dir):
    ...
    for p, thresh in zip(percentiles, thresholds):
        ...
        print(
            f"    → {pct_flagged:.1f}% predictions flagged for review ({n_flagged:,} nodes)"
        )
        print(
            f"    → Flagged MAE: {flagged_mae:.2f} vs Unflagged MAE: {unflagged_mae:.2f}"
        )
    ...
    ax.set_title(
        "(a) Predictions Requiring Review", fontweight="bold", color=COLORS["text"]
    )
    ...
    bars1 = ax.bar(
        ...
        flagged_mae,
        width,
        label="Flagged (High Uncertainty)",
        color=COLORS["coral"],
        edgecolor=COLORS["border"],
    )
    bars2 = ax.bar(
    ...
    ax.set_xlabel("Confidence Level", fontweight="bold", color=COLORS["text"])
    ax.set_ylabel(
        "Mean Absolute Error (veh/h)", fontweight="bold", color=COLORS["text"]
    )
    ax.set_title(
        "(b) Error by Flagging Status", fontweight="bold", color=COLORS["text"]
    )
    ...
    for p in [90, 95]:
        ...
        if idx:
            ...
            ax.annotate(
                f"{p}%\nMAE={mae_kept[idx]:.1f}",
                xy=(p, mae_kept[idx]),
                xytext=(p - 8, mae_kept[idx] + 10),
                fontsize=9,
                fontweight="bold",
            )

    ax.set_xlabel(
        "% Predictions Kept (Confident)", fontweight="bold", color=COLORS["text"]
    )
    ax.set_ylabel("MAE of Kept Predictions", fontweight="bold", color=COLORS["text"])
    ax.set_title(
        "(c) Accuracy-Coverage Trade-off", fontweight="bold", color=COLORS["text"]
    )
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "uq_threshold_analysis.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def analysis_2_uncertainty_heatmap(results, output_dir):
    ...
    scatter1 = ax.scatter(
        x, y, c=np.clip(uncertainties_plot, 0, unc_99), cmap=cmap_unc, s=1, alpha=0.6
    )
    ...
    ax.set_title(
        "(a) Uncertainty Heat Map - Paris Network",
        fontweight="bold",
        color=COLORS["text"],
        fontsize=12,
    )
    ...
    scatter2 = ax.scatter(
        x, y, c=np.clip(errors_plot, 0, err_99), cmap=cmap_err, s=1, alpha=0.6
    )
    ax.set_xlabel("X Coordinate (normalized)", fontweight="bold", color=COLORS["text"])
    ax.set_ylabel("Y Coordinate (normalized)", fontweight="bold", color=COLORS["text"])
    ax.set_title(
        "(b) Prediction Error Heat Map - Paris Network",
        fontweight="bold",
        color=COLORS["text"],
        fontsize=12,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "uq_spatial_heatmap.png"), dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "uq_high_uncertainty_zones.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def analysis_3_feature_error(results, output_dir):
    ...
    for i in range(n_features):
        ...
        print(
            f"  {FEATURE_NAMES[i]:<25}: ρ_unc = {corr_unc:+.4f}, ρ_error = {corr_err:+.4f}"
        )
    ...
    ax.set_xlabel(
        "Correlation with Uncertainty (ρ)", fontweight="bold", color=COLORS["text"]
    )
    ax.set_title(
        "Feature-Uncertainty Correlations", fontweight="bold", color=COLORS["text"]
    )
    ...
    plt.suptitle(
        "Feature-wise Uncertainty Analysis",
        fontsize=14,
        fontweight="bold",
        color=COLORS["text"],
        y=1.02,
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "uq_feature_analysis.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def analysis_4_calibration(results, output_dir):
    ...
    ax.set_xlabel(
        "Uncertainty Bin (Low → High)", fontweight="bold", color=COLORS["text"]
    )
    ax.set_ylabel("Value", fontweight="bold", color=COLORS["text"])
    ax.set_title(
        f"(a) Reliability Diagram (ECE = {ece:.3f})",
        fontweight="bold",
        color=COLORS["text"],
    )
    ax.legend()
    ...
    ax.set_ylabel(
        "|Error| Distribution (veh/h)", fontweight="bold", color=COLORS["text"]
    )
    ax.set_title(
        "(b) Error Distribution by Confidence", fontweight="bold", color=COLORS["text"]
    )
    ...
    ax.plot(
        cumulative_pct[sample_idx],
        cumulative_mae[sample_idx],
        color=COLORS["sky_blue"],
        linewidth=2,
    )
    ax.fill_between(
    ...
    for p in [50, 90]:
        ...
        ax.annotate(
            f"MAE={cumulative_mae[idx]:.1f}",
            xy=(p, cumulative_mae[idx]),
            xytext=(p + 5, cumulative_mae[idx] + 5),
            fontsize=9,
            fontweight="bold",
        )

    ax.set_xlabel(
        "% Most Confident Predictions", fontweight="bold", color=COLORS["text"]
    )
    ax.set_ylabel("Cumulative MAE", fontweight="bold", color=COLORS["text"])
    ax.set_title(
        "(c) Cumulative Calibration Curve", fontweight="bold", color=COLORS["text"]
    )
    ax.grid(True, alpha=0.3)
    ax.set_xlim(0, 100)
    ...
    ax.text(
        ...
        transform=ax.transAxes,
        bbox=dict(
            boxstyle="round", facecolor=COLORS["cream"], edgecolor=COLORS["border"]
        ),
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, "uq_calibration_analysis.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
    ...
    print(
        f"  Error reduction at 50% confidence: {100 * (1 - mae_top50 / mae_all):.1f}%"
    )
