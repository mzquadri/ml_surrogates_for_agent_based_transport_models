...
# Output directory
OUTPUT_DIR = Path(
    r"c:\Users\zamin\OneDrive\Desktop\Nazim_thesis\ml_surrogates_for_agent_based_transport_models\thesis\latex\figures"
)
...
def save_fig(name):
    ...
    plt.savefig(
        path, facecolor="white", edgecolor="none", bbox_inches="tight", pad_inches=0.15
    )
    plt.close()
...
def chart_trials_r2_comparison():
    ...
    ax.set_title(
        "Model Performance: Validation vs Test R² Across All Trials",
        fontweight="bold",
        pad=15,
    )
...
def chart_trials_heatmap():
    ...
    data = np.array(
        [
            ...
            [
                -TRIALS[t]["val_mae"] for t in trials
            ],  # Negative so lower is better (darker)
        ...
        for j, trial in enumerate(trials):
            if "R²" in metric:
                val = (
                    TRIALS[trial]["val_r2"]
                    if "Val" in metric
                    else TRIALS[trial]["test_r2"]
                )
                text = f"{val:.3f}"
            else:
                val = (
                    TRIALS[trial]["val_mae"]
                    if "Val" in metric
                    else TRIALS[trial]["test_mae"]
                )
                text = f"{val:.2f}"
            ax.text(
                j + 0.5,
...
def chart_generalization_gap():
    ...
    # Gap annotation
    ax.fill_between(
        [0.4, 0.65], [-0.02, -0.02], [0.4, 0.65], alpha=0.1, color=TUM_ORANGE
    )
    ax.annotate(
        "Generalization\nGap",
        xy=(0.5, 0.15),
    ...
    ax.set_title(
        "Generalization Gap: Validation vs Test Performance", fontweight="bold", pad=15
    )
...
def chart_uq_bar_comparison():
    ...
    plt.suptitle("UQ Methods Comparison", fontweight="bold", fontsize=13, y=0.98)
...
def chart_mc_dropout_scatter():
    ...
    scatter = ax.scatter(
        uncertainty, errors, c=errors, cmap="Blues", alpha=0.5, s=20, edgecolors="none"
    )
    ...
    ax.set_title(
        "MC Dropout: Uncertainty vs Prediction Error\n(3.16M predictions on 100 test graphs)",
        fontweight="bold",
        pad=15,
    )
...
def chart_ensemble_comparison():
    ...
    legend_elements = [
        mpatches.Patch(
            facecolor=TUM_GREEN, edgecolor="gold", linewidth=2, label="Best Single"
        ),
...
def chart_feature_visualization():
    ...
    plt.suptitle(
        "Input Features: Distribution Analysis (5 Features)",
        fontweight="bold",
        fontsize=13,
        y=0.98,
    )
    fig.subplots_adjust(
        left=0.08, right=0.98, top=0.9, bottom=0.08, hspace=0.35, wspace=0.25
    )
...
after policy)"""
    ...
    ax.axis("off")
    ax.set_title(
        "Road Network as Graph: Features → Prediction",
        fontweight="bold",
        fontsize=13,
        pad=20,
    )
...
def chart_summary_dashboard():
    ...
    r2 = [0.0057, -0.0021]
    bars = ax3.bar(
        models, r2, color=[TUM_GREEN, TUM_ORANGE], edgecolor="white", linewidth=2
    )
    ...
    plt.suptitle(
        "UQ for GNN Traffic Surrogates: Summary", fontweight="bold", fontsize=15, y=0.98
    )

    fig.subplots_adjust(
        left=0.08, right=0.95, top=0.9, bottom=0.08, hspace=0.3, wspace=0.2
    )
