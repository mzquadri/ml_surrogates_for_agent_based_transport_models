...
Used for Conformalized Quantile Regression (CQR), Trial 11.
...
class PointNetTransfGATFrozenCQR(nn.Module):
    """
    Frozen-backbone quantile GNN for CQR (Trial 11).

    Backbone (point_net_conv_1, point_net_conv_2, gat_graph_layers) is loaded
    from T8 checkpoint and permanently frozen:
      - all backbone parameters have requires_grad=False
      - backbone submodules are always kept in eval() mode via train() override
      - forward() runs backbone under torch.no_grad() for memory efficiency

    Only gat_quantile_head (GATConv 64->2) is trainable.
    """
    ...
    def __init__(self, t8_model_path: str, device=None):
        ...
        # ------------------------------------------------------------------
        # Trainable quantile head (newly initialised, NOT loaded from T8)
        # ------------------------------------------------------------------
        self.gat_quantile_head = GATConv(64, 2)
    ...
    def train(self, mode=True):
        """
        Override nn.Module.train() so backbone submodules always stay in
        eval() mode regardless of whether the parent is set to train or eval.
        This ensures backbone dropout is always OFF during head training.
        """
        super().train(mode)
        # Backbone always in eval: dropout OFF, BN uses running stats
        self.point_net_conv_1.eval()
        self.point_net_conv_2.eval()
        self.gat_graph_layers.eval()
        return self
    ...
    def forward(self, data):
        ...
        Returns
        -------
        q_lo : Tensor, shape [N, 1]   lower quantile
        q_hi : Tensor, shape [N, 1]   upper quantile
        """
        ...
        out = self.gat_quantile_head(x, edge_index)  # [N, 2]
        q_lo = out[:, 0:1]   # [N, 1]
        q_hi = out[:, 1:2]   # [N, 1]
        return q_lo, q_hi
    ...
    def _load_backbone(self, model_path: str, device):
        ...
        Expected mismatches (both are intentional):
          - Unexpected keys in checkpoint: 'gat_final.*'  (T8 output head, replaced)
          - Missing keys not in checkpoint: 'gat_quantile_head.*' (new head)
        ...
        # This mirrors run_ensemble_fix.py / ensemble_uq_experiments.py but is scoped
        # to gat_final.* only, because gat_graph_layers.module_6.lin.weight is already
        # in the correct format for the current PyG version and must not be remapped.
        remapped = {}
        ...
        # Validate missing keys: only gat_quantile_head.* are allowed
        bad_missing = [k for k in missing if not k.startswith('gat_quantile_head.')]
        if bad_missing:
            raise RuntimeError(
        ...
        n_head    = len([k for k in missing if k.startswith('gat_quantile_head.')])
        print(f"  T8 backbone loaded from: {model_path}")
        print(f"  Skipped gat_final keys (T8 output head, replaced by new head): {n_skipped}")
        print(f"  New gat_quantile_head keys (randomly initialised): {n_head}")

    def _freeze_backbone(self):
        """
        Freeze all backbone parameters. Only gat_quantile_head remains trainable.
        """
        for name, param in self.named_parameters():
            if not name.startswith('gat_quantile_head.'):
                param.requires_grad_(False)

        n_frozen    = sum(
            p.numel() for n, p in self.named_parameters()
            if not n.startswith('gat_quantile_head.')
        )
        n_trainable = sum(p.numel() for p in self.gat_quantile_head.parameters())
