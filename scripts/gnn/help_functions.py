...
def mc_dropout_predict(model, data, num_samples: int = 50, device: torch.device = None):
    ...
    Returns:
    - tuple: Mean predictions (numpy) and uncertainty/std (numpy) for each node or edge.
    """
    ...
    # Freeze BatchNorm layers if any exist (keeps running stats fixed)
    bn_layers = []
    ...
    # Use inference_mode for speed (faster than no_grad, disables autograd entirely)
    with torch.inference_mode():
        for _ in range(num_samples):
            pred = model(data)
            if isinstance(pred, tuple):  # If multiple outputs (e.g., mode_stats)
                pred = pred[0]
            predictions.append(pred.detach())  # Keep on device, detach from graph

    # Stack predictions on device, then compute stats
    predictions = torch.stack(predictions, dim=0)  # Shape: (num_samples, num_nodes, out_channels)
    mean_prediction = predictions.mean(dim=0)      # Mean prediction
    uncertainty = predictions.std(dim=0, unbiased=False)  # Epistemic uncertainty (std)
