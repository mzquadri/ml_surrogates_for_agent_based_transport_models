...
dataset_path = os.path.join(
    project_root, "data", "train_data", "dist_not_connected_10k_1pct"
)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run GNN model training with configurable parameters."
    )
    parser.add_argument(
        "--gnn_arch",
        type=str,
    ...
    parser.add_argument(
        "--out_channels", type=int, default=1, help="The number of output channels."
    )
    parser.add_argument(
        "--model_kwargs",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="mse",
        help="The loss function to use. Supported: mse, l1.",
    )
    parser.add_argument(
        "--use_weighted_loss",
        type=str_to_bool,
        default=False,
        help="Whether to use weighted loss (based on vol_base_case) or not.",
    )
    parser.add_argument(
        "--predict_mode_stats",
        type=str_to_bool,
        default=False,
        help="Whether to predict mode stats or not.",
    )
    parser.add_argument(
        "--use_bootstrapping",
        type=str_to_bool,
        default=False,
        help="Whether to use bootstrapping for train-validation split.",
    )
    parser.add_argument(
        "--num_epochs", type=int, default=1000, help="Number of epochs to train for."
    )
    parser.add_argument(
        "--batch_size", type=int, default=8, help="Batch size for training."
    )
    parser.add_argument(
        "--lr", type=float, default=0.001, help="The learning rate for the model."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_dropout", type=str_to_bool, default=False, help="Whether to use dropout."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=42,
        help="Random seed for model initialization (data split always uses seed=42).",
    )

    args = vars(parser.parse_args())
    set_random_seeds(seed_value=42)  # Always use 42 for data loading/splitting

    try:
        # Simplified GPU detection for Colab/standard environments
        ...
        unique_run_dir = os.path.join(
            base_dir, args["project_name"], args["unique_model_description"]
        )
        os.makedirs(unique_run_dir, exist_ok=True)

        model_save_path, path_to_save_dataloader = get_paths(
            base_dir=os.path.join(base_dir, args["project_name"]),
        ...
        # Reset seed for model initialization (different seed = different ensemble member)
        if args["seed"] != 42:
            print(
                f"[Deep Ensemble] Resetting seed to {args['seed']} for model initialization"
            )
        ...
        best_val_loss, best_epoch = gnn_instance.train_model(
            ...
            optimizer=torch.optim.AdamW(
                gnn_instance.parameters(), lr=config.lr, weight_decay=1e-4
            )
            if config.gnn_arch != "xgboost"
            else None,
            train_dl=train_dl,
            valid_dl=valid_dl,
            device=device,
