...
# ---------------------------------------------------------------------------
# Unified academic style (imported from thesis_style.py)
# ---------------------------------------------------------------------------
...
def analysis_31_reliability_diagram():
    ...
    panel_label(ax1, "(a)", x=-0.14, y=1.06)
    panel_label(ax2, "(b)", x=-0.14, y=1.06)
    ...
    bars.set_xticks(x)
    bars.set_xticklabels(level_labels, rotation=45, ha="right")
    ...
    # Add value labels on bars — place inside bars near the opening to avoid overlap
    for i, (g, xi) in enumerate(zip(gaps, x)):
        gap_pp = g * 100
        # Place label just inside the bar (near the 0-line end)
        if g < 0:
            label_y = gap_pp + 1.8  # inside bar, near top
            va_align = "bottom"
        else:
            label_y = gap_pp - 0.5
            va_align = "top"
    ...
    fig.tight_layout(pad=2.0, rect=[0, 0.04, 1, 0.93])
    fig.suptitle(
...
def analysis_32_stratified_uq():
    ...
    fig.tight_layout(pad=1.2, rect=[0.03, 0.02, 1.0, 0.93])
    fig.suptitle(
    ...
    fig2, axes2 = plt.subplots(1, 5, figsize=(16, 5.0), sharey=True)
    ...
    fig2.tight_layout(pad=1.5, rect=[0, 0.02, 1, 0.93])
    fig2.suptitle(
...
def analysis_36_t7_auroc():
    ...
    t8_comparison = {
        ...
        "k95": 11.34,
        "selective_50_mae": 2.32,
        "selective_90_mae": 3.23,
    }
    ...
    summary_lines = [
        ...
        f"{'─' * 36}",
        f"{'Spearman ρ':<22s} {rho:>7.4f} {t8_comparison['rho']:>7.4f}",
