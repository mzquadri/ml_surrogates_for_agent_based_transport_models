...
\paragraph{A note on scale.}\ Trial~8's absolute test $R^2 \approx 0.60$ on the $1{,}000$-scenario subset is below the $R^2 > 0.9$ reported by~\textcite{natterer2025ml} for the same architecture at the full $10{,}000$-scenario scale.\ The contribution here is the \emph{relative} ranking and calibration behaviour of UQ methods, which is what method-to-method comparisons on a fixed subset can establish; absolute point-accuracy at this scale is not claimed to be deployment-grade, and replicating the UQ comparisons at full scale is the most direct next step (Section~\ref{sec:limitations}).
...
\begin{enumerate}
 ...
 \item \textbf{A layered post-hoc calibration framework.}\ Split conformal achieves marginal $\mathrm{PICP}_{90} = 90.02\%$ and marginal $\mathrm{PICP}_{95} = 95.01\%$.\ Regression $\sigma$-scaling with $T = 2.887$ reduces ECE from $0.356$ to $0.034$ ($-90.5\%$) and brings $1\sigma$ coverage from $32.7\%$ to $68.0\%$.\ Adaptive conformal narrows conditional coverage from $[59.0\%, 98.1\%]$ to $[83.7\%, 96.4\%]$.
