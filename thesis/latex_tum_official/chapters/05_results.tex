...
\paragraph{Error detection.}\label{sec:auroc_results}\ Treating $\sigma$ as a classifier of large errors gives an AUROC of $0.7548$ at the top-10\% threshold and $0.7324$ at the top-20\%, both computed on the full $3{,}163{,}500$-node test set.\ Both metrics replicate on Trial~7 ($0.7416$ and $0.7151$; see the footnote in Section~\ref{sec:mc_results}).\ One caveat for both AUROC and the selective-prediction MAE reduction is target imbalance: a full-array audit finds $872{,}540$ exact-zero $\Delta v$ targets ($27.6\%$), and many additional changes are small.\ Consequently, pooled AUROC and MAE-on-retained-nodes remain easier than evaluation in the policy-relevant Q4 stratum (Section~\ref{sec:stratified_results}), where errors are much larger.
